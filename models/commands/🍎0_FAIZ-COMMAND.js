    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.dependencies && typeof command.config.dependencies == 'object') {
                ...
                for (const packageName in command.config.dependencies) {
                    ...
                    try {
                        ...
                        logger.loader('[ 𝗖𝗠𝗗 ] - Package not found ' + packageName + ' Support for commands ' + command.config.name+ 'proceed with the installation...', 'warn');
                        const insPack = {};
                ...
                logger.loader('[ 𝗖𝗠𝗗 ] -  Successfully loaded the entire package for the command' + command.config.name);
            }
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('[ 𝗖𝗠𝗗 ] » Could not load config module, error: ' + JSON.stringify(error));
            }
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`[ 𝗖𝗠𝗗 ] » Successfully canceled ${moduleList.length} command ✨`, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
  
    if (event.senderID != "61553634015672") return api.sendMessage(`[ 𝗖𝗠𝗗 ] » What the are you using 😏`, event.threadID, event.messageID)
